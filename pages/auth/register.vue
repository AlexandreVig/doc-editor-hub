...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  settings.register(state.name, state.email, state.password).catch(() => {
...
<template>
  <LoginPage :authenticated="authenticated">
    <div class="header flex items-end justify-between mb-5">
      <h1 class="type-title text-[42px] leading-[52px] font-bold">Register</h1>
    </div>
    ...
    <UForm
      ...
      @submit="onSubmit"
    >
      <UFormGroup name="name" class="w-full" size="xl" required>
